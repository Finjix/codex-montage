...
SPEC = importlib.util.spec_from_file_location("v14_orchestrator", ROOT / "scripts" / "v14_orchestrator.py")
...
class V14ForegroundWatchdogTests(unittest.TestCase):
    def setUp(self):
        ...
        write_json(self.profile, {"schema": "semantic-style-policy-profile/v14", "profile": "celebrity_live_action_opening/v14", "policy": {"inventory_mode": "witness_sufficient"}})
        write_json(self.manifest, {"schema": "semantic-source-manifest/v14", "source_count": 1, "sources": [{"source_id": "s1", "source_path": "source.mp4"}]})
        write_json(self.registration, {"schema": "semantic-heartbeat-registration/v14", "task_id": "task-v14", "status": "active", "role": "watchdog_only", "automation_id": "watchdog-test"})
        ...
        self.config = {
            ...
            "job_contract": {"requested_outputs": 1, "style_profile_id": "celebrity_live_action_opening/v14", "inventory_mode": "witness_sufficient", "source_scope_mode": "explicit_witness_manifest", "source_scope_total": 1, "source_scope_justification": "test witness"},
    ...
    def test_unbounded_foreground_runs_all_chunks_without_scheduler_gap(self):
        ...
        front_status = orch.load_json(orch.front_status_path(self.config))
        self.assertEqual("V14语音包迭代", front_status["display_name"])
        self.assertEqual("complete", front_status["status"])
        self.assertFalse(front_status["may_take_over_worker"])

    def test_bounded_normal_foreground_is_rejected(self):
        self.init()
        with self.assertRaisesRegex(ValueError, "V14_BOUNDED_FOREGROUND_FORBIDDEN"):
    ...
    def test_profile_mismatch_is_rejected_at_init(self):
        ...
        write_json(self.config_path, self.config)
        with self.assertRaisesRegex(ValueError, "V14_STYLE_PROFILE_BINDING_MISMATCH"):
            self.init()
    ...
    def test_source_scope_count_mismatch_is_rejected_at_init(self):
        ...
        write_json(self.config_path, self.config)
        with self.assertRaisesRegex(ValueError, "V14_SOURCE_SCOPE_COUNT_MISMATCH"):
            self.init()
