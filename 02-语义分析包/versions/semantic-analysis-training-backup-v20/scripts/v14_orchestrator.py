#!/usr/bin/env python3
"""V14 foreground-continuous orchestrator with watchdog-only heartbeat recovery."""
...
SCHEMA = "semantic-production-orchestrator-state/v14"
CONFIG_SCHEMA = "semantic-production-pipeline-config/v14"
MODEL_RESPONSE_SCHEMA = "semantic-model-phase-response/v14"
LEASE_SCHEMA = "semantic-foreground-lease/v14"
FRONT_STATUS_SCHEMA = "semantic-front-status/v14"
...
def save_state(path: Path, state: dict, backup: bool = True) -> None:
    if backup and path.is_file():
        backup_dir = path.parent / ".v14" / "state_backups"
...
def verify_heartbeat_registration(config: dict) -> None:
    ...
    if registration.get("schema") != "semantic-heartbeat-registration/v14" or registration.get("task_id") != config.get("task_id") or registration.get("status") != "active" or registration.get("role") != "watchdog_only" or not registration.get("automation_id"):
...
def run_local_heartbeat_slice(config: dict, state_path: Path, state: dict, phase: dict) -> tuple[int, dict]:
    ...
    command = expand_command(phase_command(phase, attempt), config, state_path)
    log_dir = Path(config["task_root"]) / ".v14" / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
...
def foreground_lease_path(config: dict) -> Path:
    return Path(config["task_root"]) / ".v14" / "foreground_lease.json"


def front_status_path(config: dict) -> Path:
    return Path(config["task_root"]) / ".v14" / "front_status.json"
...
def write_front_status(config: dict, state: dict, run_id: str | None, started_at: str | None, display_status: str) -> dict:
    ...
    payload = {
        ...
        "display_name": str(config.get("display_name", "V14语音包迭代")),
...
def front_status_snapshot(config: dict, state: dict) -> dict:
    ...
    if not path.is_file():
        return {"schema": FRONT_STATUS_SCHEMA, "display_name": str(config.get("display_name", "V14语音包迭代")), "status": "not_started", "may_take_over_worker": False}
