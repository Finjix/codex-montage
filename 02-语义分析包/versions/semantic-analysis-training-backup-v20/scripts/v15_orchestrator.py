...
SCHEMA = "semantic-production-orchestrator-state/v18"
CONFIG_SCHEMA = "semantic-production-pipeline-config/v15"
MODEL_RESPONSE_SCHEMA = "semantic-model-phase-response/v18"
LEASE_SCHEMA = "semantic-foreground-lease/v15"
FRONT_STATUS_SCHEMA = "semantic-front-status/v15"
...
def validate_config(config: dict) -> None:
    ...
    if config.get("mode", "production") == "production":
        ...
        if content_contract.get("schema") not in {"celebrity-product-content-grounding-contract/v18", "celebrity-product-content-grounding-contract/v19"} or content_contract.get("contract_id") != job["content_grounding_contract_id"]:
...
def verify_heartbeat_registration(config: dict) -> None:
    ...
    if registration.get("schema") != "semantic-heartbeat-registration/v15" or registration.get("task_id") != config.get("task_id") or registration.get("status") != "active" or registration.get("role") != "watchdog_only" or not registration.get("automation_id"):
