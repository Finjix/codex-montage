#!/usr/bin/env python3
"""V13 foreground-continuous orchestrator with watchdog-only heartbeat recovery."""
...
SCHEMA = "semantic-production-orchestrator-state/v13"
CONFIG_SCHEMA = "semantic-production-pipeline-config/v13"
MODEL_RESPONSE_SCHEMA = "semantic-model-phase-response/v13"
LEASE_SCHEMA = "semantic-foreground-lease/v13"
FRONT_STATUS_SCHEMA = "semantic-front-status/v13"
...
def save_state(path: Path, state: dict, backup: bool = True) -> None:
    if backup and path.is_file():
        backup_dir = path.parent / ".v13" / "state_backups"
...
def validate_config(config: dict) -> None:
    ...
    if "CANDIDATE_BUILD" in names:
        ...
        if missing:
            raise ValueError("V13_SOURCE_FRAME_PIPELINE_MISSING:" + ",".join(missing))
        positions = [names.index(name) for name in required_order]
        if positions != sorted(positions):
            raise ValueError("V13_SOURCE_FRAME_PIPELINE_ORDER_INVALID")
        if not isinstance(config.get("variables", {}).get("source_frame_evidence_index"), str):
            raise ValueError("V13_SOURCE_FRAME_INDEX_VARIABLE_REQUIRED")
    if config.get("mode", "production") == "production":
        ...
        if not isinstance(execution, dict) or execution.get("foreground_required") is not True or execution.get("heartbeat_recovery_only") is not True:
            raise ValueError("V13_REQUIRES_FOREGROUND_AND_WATCHDOG_ONLY_HEARTBEAT")
        ...
        if not isinstance(job, dict):
            raise ValueError("V13_JOB_CONTRACT_REQUIRED")
        ...
        if profile_policy.get("inventory_mode") != job["inventory_mode"]:
            raise ValueError("V13_PROFILE_INVENTORY_MODE_MISMATCH")
        ...
        if job["source_scope_mode"] != expected_scope_mode:
            raise ValueError("V13_SOURCE_SCOPE_MODE_INVENTORY_MISMATCH")
        if not manifest_path.is_file():
            raise ValueError("V13_SOURCE_SCOPE_MANIFEST_MISSING")
        ...
        if not isinstance(sources, list) or len(sources) != int(job["source_scope_total"]) or int(manifest.get("source_count", -1)) != len(sources):
            raise ValueError("V13_SOURCE_SCOPE_COUNT_MISMATCH")
        if str(variables.get("source_scope_total")) != str(job["source_scope_total"]):
            raise ValueError("V13_SOURCE_SCOPE_VARIABLE_MISMATCH")
        if str(variables.get("requested_outputs")) != str(job["requested_outputs"]):
            raise ValueError("V13_REQUESTED_OUTPUTS_VARIABLE_MISMATCH")
...
def verify_heartbeat_registration(config: dict) -> None:
    ...
    if registration.get("schema") != "semantic-heartbeat-registration/v13" or registration.get("task_id") != config.get("task_id") or registration.get("status") != "active" or registration.get("role") != "watchdog_only" or not registration.get("automation_id"):
...
def run_local_heartbeat_slice(config: dict, state_path: Path, state: dict, phase: dict) -> tuple[int, dict]:
    ...
    command = expand_command(phase_command(phase, attempt), config, state_path)
    log_dir = Path(config["task_root"]) / ".v13" / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
...
def foreground_lease_path(config: dict) -> Path:
    return Path(config["task_root"]) / ".v13" / "foreground_lease.json"


def front_status_path(config: dict) -> Path:
    return Path(config["task_root"]) / ".v13" / "front_status.json"
...
def write_front_status(config: dict, state: dict, run_id: str | None, started_at: str | None, display_status: str) -> dict:
    ...
    payload = {
        ...
        "display_name": str(config.get("display_name", "V13语音包迭代")),
...
def front_status_snapshot(config: dict, state: dict) -> dict:
    ...
    if not path.is_file():
        return {"schema": FRONT_STATUS_SCHEMA, "display_name": str(config.get("display_name", "V13语音包迭代")), "status": "not_started", "may_take_over_worker": False}
...
def acquire_foreground_lease(config: dict, state: dict, mode: str) -> dict:
    ...
    if path.is_file():
        ...
        if existing.get("schema") == LEASE_SCHEMA and existing.get("status") == "running" and pid_alive(existing_pid) and int(existing_pid) != os.getpid() and age_seconds(existing.get("renewed_at")) <= stale_after:
            raise ValueError("V13_FOREGROUND_ALREADY_ACTIVE")
...
def run_continuous(config_path: Path, state_path: Path, budget_seconds: float, recovery: bool = False) -> tuple[int, dict]:
    ...
    if not recovery and execution.get("foreground_required") is True and budget_seconds > 0:
        raise ValueError("V13_BOUNDED_FOREGROUND_FORBIDDEN_USE_RECOVERY_MODE")
