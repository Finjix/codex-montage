# V14 foreground and watchdog contract

Normal production is `scripts/v14_orchestrator.py run-continuous --config ... --state ...` with no positive budget. A positive normal budget is rejected with `V14_BOUNDED_FOREGROUND_FORBIDDEN_USE_RECOVERY_MODE`. The foreground process writes and renews `.v14/foreground_lease.json` after every durable slice and releases the lease on model handoff, completion or failure.
