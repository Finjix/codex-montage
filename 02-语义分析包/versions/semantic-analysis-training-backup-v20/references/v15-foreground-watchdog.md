# V15 foreground and watchdog contract

Normal production is `scripts/v15_orchestrator.py run-continuous --config ... --state ...` with no positive budget. A positive normal budget is rejected with `V15_BOUNDED_FOREGROUND_FORBIDDEN_USE_RECOVERY_MODE`. The foreground process writes and renews `.v15/foreground_lease.json` after every durable slice and releases the lease on model handoff, completion or failure.
